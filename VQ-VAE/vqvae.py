...
# copied from https://github.com/MishaLaskin/vqvae
...
class MyTrainer(pl.LightningModule):
    def __init__(self):
        super().__init__()
        self.model = VQVAE(h_dim=128, res_h_dim=32, n_res_layers=2, n_embeddings=512, embedding_dim=64, beta=0.25).to(device)

    def configure_optimizers(self):
        optimizer = optim.Adam(self.model.parameters(), lr=3e-4, amsgrad=True)
        return optimizer

    def training_step(self, x, x_id):
        x = x[0]
...
x_train_var = np.var(trainset.data / 255.0)
...
trainer = pl.Trainer(limit_train_batches=100, max_epochs=10, devices='auto')
